...
U8G2_SSD1306_128X64_NONAME_F_SW_I2C oled2(U8G2_R0, 14, 17, U8X8_PIN_NONE); // Software I2C on pins 17(SDA), 14(SCL)

// ---------------------- Stepper (Pins adjusted to match example_code.ino's working configuration) ----------------------
AccelStepper stepper1(AccelStepper::HALF4WIRE, 25, 32, 33, 5); // Corrected pin order: IN1, IN3, IN2, IN4 (25, 32, 33, 5)
AccelStepper stepper2(AccelStepper::HALF4WIRE, 23, 18, 19, 4); // Corrected pin order: IN1, IN3, IN2, IN4 (23, 18, 19, 4)
...
// ---------------------- Drum geometry ----------------------
const int totalSlots = 8;                            // Added missing definition for total slots in the drum
#define STEPS_PER_REV 4552                           // Kept the correct value, removed the duplicate
const int stepsPerSlot = STEPS_PER_REV / totalSlots; // Now compiles correctly
...
void loop()
{
    ...
    for (int d = 0; d < 2; d++)
    {
        if (drumIsEffectivelyEmpty[d] && millis() - lastEmptyCheckTime[d] > EMPTY_CHECK_INTERVAL)
        {
            ...
            bool refilled = false;
            for (int s = 1; s <= (totalSlots - 1); s++) // Use totalSlots constant
            {
...
void initSteppers()
{
    ...
    logInfo("Steppers 1 & 2 initialized with max speed and acceleration.");
}
void initIO()
{
    ...
    server.on("/api/history", HTTP_GET, [](AsyncWebServerRequest *request)
        ...
        for (int i = 0; i < total; i++) {
            ...
            if (openBracket == 0 && closeBracket > 0 && closeBracket + 2 <= (int)line.length()) {
                ...
                line.trim(); // Trim after extracting timestamp
    ...
    server.on("/api/upcoming-schedules", HTTP_GET, [](AsyncWebServerRequest *request)
        ...
        for (int day = 0; day < 2; day++) {
            ...
            for (int i = 0; i < dailyScheduleCount; i++) {
                ...
                if (shouldInclude) {
                    JsonObject schedule = schedules.add<JsonObject>(); // Use JsonObject for clarity
        ...
        if (request->hasParam("data", true)) {
            ...
            if (deserializeJson(doc, data).code() == DeserializationError::Ok) {
                ...
                for (JsonObject med : medicines) {
                    if (nextSlot <= (totalSlots - 1)) { // Use totalSlots constant
...
void dispensePill(int drum, int pills)
{
    ...
    if (drum == 1)
    {
        ...
        logInfo("Moving stepper1 by " + String(stepsPerSlot * pills) + " steps.");
        stepper1.move(stepsPerSlot * pills);
    }
    else if (drum == 2)
    {
        currentState = DISPENSING_DRUM2;
        logInfo("Moving stepper2 by " + String(stepsPerSlot * pills) + " steps.");
...
void handleDispenseFinished(int drum, int pills)
{
    logInfo("handleDispenseFinished called for Drum " + String(drum) + " after dispensing " + String(pills) + " pill(s).");

    int *currentSlotPtr = (drum == 1) ? &currentSlotDrum1 : &currentSlotDrum2;
    String pillName = getPillName(drum, *currentSlotPtr); // Get pill name for the starting slot of this dispense operation
    ...
    *currentSlotPtr = ((*currentSlotPtr - 1 + pills) % (totalSlots - 1)) + 1;
...
static void appendHistoryItemJson(String &out, const String &timestampRaw, const String &message, bool &first)
{
    ...
    if (medicine.length() > 0)
        itemDoc["medicine"] = medicine;
    else
        itemDoc["medicine"] = "";
    if (drum >= 0)
        itemDoc["drum"] = drum;
    else
        itemDoc["drum"] = nullptr;
    if (slot >= 0)
        itemDoc["slot"] = slot;
    else
        itemDoc["slot"] = nullptr;
    if (details.length() > 0)
        itemDoc["details"] = details;
    else
        itemDoc["details"] = "";
...
void showTakenOnOled(int drum, const String &pill)
{
    ...
    String line2 = pill.substring(2, 15);
    bool displaySuccess = false;
    int retryCount = 0;
    ...
    while (!displaySuccess && retryCount < 3)
    {
        if (drum == 1)
        {
            if (xSemaphoreTake(i2c_mutex, pdMS_TO_TICKS(1000)) == pdTRUE)
            {
                ...
                oled1.setFont(u8g2_font_ncenB08_tr);
                oled1.drawStr(0, 20, line1.c_str());
                oled1.drawStr(10, 40, line2.c_str());
                oled1.sendBuffer();
                xSemaphoreGive(i2c_mutex);
                displaySuccess = true;
            }
        ...
        else
        {
            if (xSemaphoreTake(i2c_mutex, pdMS_TO_TICKS(1000)) == pdTRUE)
            {
                ...
                oled2.setFont(u8g2_font_ncenB08_tr);
                oled2.drawStr(0, 20, line1.c_str());
                oled2.drawStr(10, 40, line2.c_str());
                oled2.sendBuffer();
                xSemaphoreGive(i2c_mutex);
                displaySuccess = true;
            }
...
bool updateOledDisplay(int drum, const String &line1, const String &line2)
{
    ...
    while (!success && retryCount < 3)
    {
        ...
        {
            // Software I2C OLED also needs mutex protection if it can be accessed from multiple tasks
            if (xSemaphoreTake(i2c_mutex, pdMS_TO_TICKS(1000)) == pdTRUE)
            {
                oled2.clearBuffer();
                oled2.setFont(u8g2_font_ncenB08_tr);
